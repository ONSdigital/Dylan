...
node {
    ...
    stage('Bundle') {
        ...
        sh "tar -cvzf dylan-${revision}.tar.gz appspec.yml scripts/codedeploy"
        sh "aws s3 cp dylan-${revision}.tar.gz s3://${env.S3_REVISIONS_BUCKET}/dylan-${revision}.tar.gz"
    }
...
def registry(branch, tag) {
    [
        hub: [
            ...
            image: "${env.DOCKERHUB_REPOSITORY}/dylan",
            tag: 'live',
